...
def get_redfin_data(zipcode):
    ...
    for prop in properties[:1]:  # Limiting to 1 for testing
        # print(prop)
        ...
        latitude, longitude, url = None, None, None
        if script_tag:
        ...
        property_details = {
            ...
            'price': extract_numeric_value(price_elem.text.strip()) if price_elem else 0,
            ...
            'lot_size': extract_numeric_value(lot_size_elem.text.strip()) if lot_size_elem else 0,
            'coordinates': {
                'latitude': latitude,
                'longitude': longitude
            },
